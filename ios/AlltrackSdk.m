...
RCT_EXPORT_METHOD(create:(NSDictionary *)dict) {
    ...
    ALTConfig *alltrackConfig = [ALTConfig configWithAppToken:appToken environment:environment allowSuppressLogLevel:allowSuppressLogLevel];
    ...
    if (_isAttributionCallbackImplemented
        ...
        [alltrackConfig setDelegate:
         [AlltrackSdkDelegate getInstanceWithSwizzleOfAttributionCallback:_isAttributionCallbackImplemented
    ...
    if ([self isFieldValid:allowiAdInfoReading]) {
        [alltrackConfig setAllowiAdInfoReading:[allowiAdInfoReading boolValue]];
    }
...
RCT_EXPORT_METHOD(setOfflineMode:(NSNumber * _Nonnull)isEnabled) {
    [Alltrack setOfflineMode:[isEnabled boolValue]];
}
...
RCT_EXPORT_METHOD(getAppTrackingAuthorizationStatus:(RCTResponseSenderBlock)callback) {
    callback(@[@([Alltrack appTrackingAuthorizationStatus])]);
}
...
RCT_EXPORT_METHOD(convertUniversalLink:(NSString *)urlString scheme:(NSString *)scheme callback:(RCTResponseSenderBlock)callback) {
    ...
    NSURL *converted = [Alltrack convertUniversalLink:url scheme:scheme];
...
RCT_EXPORT_METHOD(trackThirdPartySharing:(NSDictionary *)dict) {
    ...
    ALTThirdPartySharing *alltrackThirdPartySharing = [[ALTThirdPartySharing alloc] initWithIsEnabledNumberBool:isEnabled];
    ...
    if ([self isFieldValid:partnerSharingSettings]) {
        for (int i = 0; i < [partnerSharingSettings count]; i += 3) {
            ...
            [alltrackThirdPartySharing addPartnerSharingSetting:partnerName key:key value:[value boolValue]];
        }
    }
