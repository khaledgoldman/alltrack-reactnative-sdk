...
@implementation ALTSystemProfile
...
+ (NSString *)cpuFamily {
    ...
    if (error != 0) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain CPU family (%d)", error];
        return nil;
    }
    ...
    [ALTAlltrackFactory.logger warn:@"%@", unknowCpuFamily];
...
+ (int)cpuCount {
    ...
    if (error != 0) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain CPU count (%d)", error];
        return 1;
    }
...
+ (NSString *)machineArch {
    return [ALTSystemProfile readSysctlbByNameString:"hw.machinearch" errorLog:@"Failed to obtain machine arch"];
}

+ (NSString *)machineModel {
    return [ALTSystemProfile readSysctlbByNameString:"hw.model" errorLog:@"Failed to obtain machine model"];
}

+ (NSString *)cpuBrand {
    return [ALTSystemProfile readSysctlbByNameString:"machdep.cpu.brand_string" errorLog:@"Failed to obtain CPU brand"];
}

+ (NSString *)cpuFeatures {
    return [ALTSystemProfile readSysctlbByNameString:"machdep.cpu.features" errorLog:@"Failed to obtain CPU features"];
}

+ (NSString *)cpuVendor {
    return [ALTSystemProfile readSysctlbByNameString:"machdep.cpu.vendor" errorLog:@"Failed to obtain CPU vendor"];
}

+ (NSString *)osVersion {
    return [ALTSystemProfile readSysctlbByNameString:"kern.osversion" errorLog:@"Failed to obtain OS version"];
}
...
+ (NSString *)readSysctlbByNameString:(const char*)name
    ...
    if (error != 0) {
        [ALTAlltrackFactory.logger error:@"%@ (%d)", errorLog, error];
        return nil;
    }
...
+ (NSString *)appleLanguage {
    ...
    if ([languages count] == 0) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain preferred language"];
        return nil;
    }
...
+ (long long)cpuSpeed {
    ...
    if (error) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain CPU speed (%d)", error];
        return -1;
    }
...
+ (long long)ramsize {
	...
	if (error) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain RAM size (%d)", error];
        return -1;
	}
...
+ (NSString *)cpuType {
    ...
    if (error != 0) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain CPU type (%d)", error];
        return nil;
    }

    NSString *cpuTypeString = [ALTSystemProfile readCpuTypeSubtype:cputype readSubType:NO cpusubtype:0];
    if (cpuTypeString != nil) {
        return cpuTypeString;
    }

    NSString *unknowCpuType = [NSString stringWithFormat:@"Unknown CPU type %d", cputype];
    [ALTAlltrackFactory.logger warn:@"%@", unknowCpuType];
...
+ (NSString *)cpuSubtype {
    ...
    if (error != 0) {
        [ALTAlltrackFactory.logger error:@"Failed to obtain CPU subtype (%d)", error];
        return nil;
    }

    NSString *cpuSubtypeString = [ALTSystemProfile readCpuTypeSubtype:cputype readSubType:YES cpusubtype:cpuSubtype];
    ...
    [ALTAlltrackFactory.logger warn:@"%@", unknowCpuSubtype];
