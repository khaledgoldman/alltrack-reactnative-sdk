...
@implementation ALTActivityState
...
- (id)init {
    ...
    [self assignRandomToken:[ALTUtil generateRandomUuid]];
...
- (void)assignRandomToken:(NSString *)randomToken {
    NSString *persistedDedupeToken = [ALTUtil getPersistedRandomToken];
    if (persistedDedupeToken != nil) {
        if ((bool)[[NSUUID alloc] initWithUUIDString:persistedDedupeToken]) {
            [[ALTAlltrackFactory logger] verbose:@"Primary dedupe token successfully read"];
    ...
    self.isPersisted = [ALTUtil setPersistedRandomToken:self.dedupeToken];
}
...
- (id)copyWithZone:(NSZone *)zone {
    ALTActivityState *copy = [[[self class] allocWithZone:zone] init];
