...
@implementation ALTEvent

+ (ALTEvent *)eventWithEventToken:(NSString *)eventToken {
    return [[ALTEvent alloc] initWithEventToken:eventToken];
}
...
- (void)setReceipt:(NSData *)receipt transactionId:(NSString *)transactionId {
    ...
    if ([ALTUtil isNull:receipt] || [receipt length] == 0) {
        _emptyReceipt = YES;
    }
    _receipt = receipt;
...
- (BOOL)checkReceipt:(NSData *)receipt transactionId:(NSString *)transactionId {
    if ([ALTUtil isNotNull:receipt] && [ALTUtil isNull:transactionId]) {
...
- (id)copyWithZone:(NSZone *)zone {
    ALTEvent *copy = [[[self class] allocWithZone:zone] init];
