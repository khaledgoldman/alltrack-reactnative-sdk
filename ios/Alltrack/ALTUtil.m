...
static NSString * const kClientSdk                  = @"ios0.0.1";
static NSString * const kDeeplinkParam              = @"deep_link=";
static NSString * const kSchemeDelimiter            = @"://";
static NSString * const kDefaultScheme              = @"AlltrackUniversalScheme";
static NSString * const kUniversalLinkPattern       = @"https://[^.]*\\.ulink\\.alltrack\\.com/ulink/?(.*)";
static NSString * const kOptionalRedirectPattern    = @"alltrack_redirect=[^&#]*";
static NSString * const kShortUniversalLinkPattern  = @"http[s]?://[a-z0-9]{4}\\.(?:[a-z]{2}\\.)?alt\\.st/?(.*)";
...
+ (void)initializeUniversalLinkRegex {
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:kUniversalLinkPattern
                                                                           options:NSRegularExpressionCaseInsensitive
                                                                             error:&error];
    if ([ALTUtil isNotNull:error]) {
        [ALTAlltrackFactory.logger error:@"Universal link regex rule error (%@)", [error description]];
        return;
    }
...
+ (void)initializeShortUniversalLinkRegex {
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:kShortUniversalLinkPattern
                                                                           options:NSRegularExpressionCaseInsensitive
                                                                             error:&error];
    if ([ALTUtil isNotNull:error]) {
        [ALTAlltrackFactory.logger error:@"Short Universal link regex rule error (%@)", [error description]];
        return;
    }
...
+ (void)initializeOptionalRedirectRegex {
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:kOptionalRedirectPattern
                                                                           options:NSRegularExpressionCaseInsensitive
                                                                             error:&error];
    if ([ALTUtil isNotNull:error]) {
        [ALTAlltrackFactory.logger error:@"Optional redirect regex rule error (%@)", [error description]];
        return;
    }
...
+ (void)initializeExcludedDeeplinkRegex {
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:kExcludedDeeplinksPattern
                                                                           options:NSRegularExpressionCaseInsensitive
                                                                             error:&error];
    if ([ALTUtil isNotNull:error]) {
        [ALTAlltrackFactory.logger error:@"Excluded deep link regex rule error (%@)", [error description]];
        return;
    }
...
+ (void)updateUrlSessionConfiguration:(ALTConfig *)config {
...
+ (NSDateFormatter *)getDateFormatter {
    ...
    Class class = NSClassFromString([NSString altJoin:@"N", @"S", @"locale", nil]);
    if (class != nil) {
        NSString *keyLwli = [NSString altJoin:@"locale", @"with", @"locale", @"identifier:", nil];
...
+ (NSString *)formatDate:(NSDate *)value {
    NSDateFormatter *dateFormatter = [ALTUtil getDateFormatter];
...
+ (id)readObject:(NSString *)fileName
    ...
    @synchronized(syncObject) {
        NSString *documentsFilePath = [ALTUtil getFilePathInDocumentsDir:fileName];
        NSString *appSupportFilePath = [ALTUtil getFilePathInAppSupportDir:fileName];
        ...
        @try {
            ...
            if (@available(iOS 11.0, tvOS 11.0, *)) {
                ...
                if (errorUnarchiver == nil) {
                    ...
                    // [[ALTAlltrackFactory logger] debug:@"Failed to read %@ with error: %@", objectName, errorUnarchiver.localizedDescription];
                }
            } else {
#pragma clang diagnostic push
            ...
            if (appSupportObject != nil) {
                if ([appSupportObject isKindOfClass:classToRead]) {
                    ...
                    if ([appSupportObject isKindOfClass:[NSArray class]]) {
                        [[ALTAlltrackFactory logger] debug:@"Package handler read %d packages", [appSupportObject count]];
                    } else {
                        [[ALTAlltrackFactory logger] debug:@"Read %@: %@", objectName, appSupportObject];
                    }

                    // Just in case check if old file exists in Documents folder and if yes, remove it.
                    [ALTUtil deleteFileInPath:documentsFilePath];

                    return appSupportObject;
                }
            } else {
                // [[ALTAlltrackFactory logger] error:@"Failed to read %@ file", appSupportFilePath];
                [[ALTAlltrackFactory logger] debug:@"File %@ not found in \"Application Support/Alltrack\" folder", fileName];
            }
        } @catch (NSException *ex) {
            // [[ALTAlltrackFactory logger] error:@"Failed to read %@ file  (%@)", appSupportFilePath, ex];
            [[ALTAlltrackFactory logger] error:@"Failed to read %@ file from \"Application Support/Alltrack\" folder (%@)", fileName, ex];
        }
...
#pragma clang diagnostic pop
            ...
            if (documentsObject != nil) {
                ...
                if ([documentsObject isKindOfClass:[NSArray class]]) {
                    [[ALTAlltrackFactory logger] debug:@"Package handler read %d packages", [documentsObject count]];
                } else {
                    [[ALTAlltrackFactory logger] debug:@"Read %@: %@", objectName, documentsObject];
                }

                // Do the file migration.
                [[ALTAlltrackFactory logger] verbose:@"Migrating %@ file from Documents to \"Application Support/Alltrack\" folder", fileName];
                [ALTUtil migrateFileFromPath:documentsFilePath toPath:appSupportFilePath];

                return documentsObject;
            } else {
                // [[ALTAlltrackFactory logger] error:@"Failed to read %@ file", documentsFilePath];
                [[ALTAlltrackFactory logger] debug:@"File %@ not found in Documents folder", fileName];
            }
        } @catch (NSException *ex) {
            // [[ALTAlltrackFactory logger] error:@"Failed to read %@ file (%@)", documentsFilePath, ex];
            [[ALTAlltrackFactory logger] error:@"Failed to read %@ file from Documents folder (%@)", fileName, ex];
        }
...
+ (void)writeObject:(id)object
    ...
    @synchronized(syncObject) {
        @try {
            BOOL result;
            NSString *filePath = [ALTUtil getFilePathInAppSupportDir:fileName];
            if (!filePath) {
                [[ALTAlltrackFactory logger] error:@"Cannot get filepath from filename: %@, to write %@ file", fileName, objectName];
                return;
            }
...
#pragma clang diagnostic pop
            ...
            if (result == YES) {
                [ALTUtil excludeFromBackup:filePath];
                if ([object isKindOfClass:[NSArray class]]) {
                    [[ALTAlltrackFactory logger] debug:@"Package handler wrote %d packages", [object count]];
                } else {
                    [[ALTAlltrackFactory logger] debug:@"Wrote %@: %@", objectName, object];
                }
            } else {
                [[ALTAlltrackFactory logger] error:@"Failed to write %@ file", objectName];
            }
        } @catch (NSException *exception) {
            [[ALTAlltrackFactory logger] error:@"Failed to write %@ file (%@)", objectName, exception];
        }
    }
}
...
+ (BOOL)migrateFileFromPath:(NSString *)oldPath toPath:(NSString *)newPath {
    ...
    Class class = NSClassFromString([NSString altJoin:@"N", @"S", @"file", @"manager", nil]);
    if (class == nil) {
        return NO;
    }
    NSString *keyDm = [NSString altJoin:@"default", @"manager", nil];
...
#pragma clang diagnostic pop
    NSString *keyCpy = [NSString stringWithFormat:@"%@%@%@",
                        [NSString altJoin:@"copy", @"item", @"at", @"path", @":", nil],
                        [NSString altJoin:@"to", @"path", @":", nil],
                        [NSString altJoin:@"error", @":", nil]];
    ...
    if (error != nil) {
        [[ALTAlltrackFactory logger] error:@"Error while copying from %@ to %@", oldPath, newPath];
        [[ALTAlltrackFactory logger] error:[error description]];
        return NO;
    }
...
+ (NSString *)getFilePathInAppSupportDir:(NSString *)fileName {
    ...
    // All Alltrack files are going to be stored in Alltrack sub-directory inside Application Support directory.
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES);
    NSString *appSupportDir = [paths firstObject];
    NSString *alltrackDirName = @"Alltrack";
    if (![ALTUtil checkForDirectoryPresenceInPath:appSupportDir forFolder:[appSupportDir lastPathComponent]]) {
        return nil;
    }
    NSString *alltrackDir = [appSupportDir stringByAppendingPathComponent:[NSString stringWithFormat:@"/%@", alltrackDirName]];
    if (![ALTUtil checkForDirectoryPresenceInPath:alltrackDir forFolder:alltrackDirName]) {
        return nil;
    }
    NSString *filePath = [alltrackDir stringByAppendingPathComponent:fileName];
    return filePath;
}
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    id man = [class performSelector:selDm];
#pragma clang diagnostic pop
    NSString *keyExi = [NSString altJoin:@"file", @"exists", @"at", @"path", @":", nil];
    ...
    if (!exists) {
        [[ALTAlltrackFactory logger] debug:@"%@ directory not present and will be created", folderName];
        BOOL withIntermediateDirectories = NO;
        NSDictionary *attributes = nil;
        __autoreleasing NSError *error;
        __autoreleasing NSError **errorPointer = &error;
        NSString *keyCrt = [NSString stringWithFormat:@"%@%@%@%@",
                            [NSString altJoin:@"create", @"directory", @"at", @"path", @":", nil],
                            [NSString altJoin:@"with", @"intermediate", @"directories", @":", nil],
                            [NSString altJoin:@"attributes", @":", nil],
                            [NSString altJoin:@"error", @":", nil]];
        ...
        if (error != nil) {
            [[ALTAlltrackFactory logger] error:@"Error while creating %@ directory", path];
            [[ALTAlltrackFactory logger] error:[error description]];
            return NO;
        }
    }
...
+ (NSString *)queryString:(NSDictionary *)parameters {
    return [ALTUtil queryString:parameters queueSize:0];
}
...
+ (NSString *)queryString:(NSDictionary *)parameters
    ...
    for (NSString *key in parameters) {
        ...
        NSString *escapedValue = [value altUrlEncode];
        NSString *escapedKey = [key altUrlEncode];
        NSString *pair = [NSString stringWithFormat:@"%@=%@", escapedKey, escapedValue];
        [pairs addObject:pair];
    }

    double now = [NSDate.date timeIntervalSince1970];
    NSString *dateString = [ALTUtil formatSeconds1970:now];
    NSString *escapedDate = [dateString altUrlEncode];
    ...
    if (queueSize > 0) {
        ...
        NSString *escapedQueueSize = [queueSizeString altUrlEncode];
...
+ (NSDictionary *)convertDictionaryValues:(NSDictionary *)dictionary {
    ...
    for (NSString *key in dictionary) {
        ...
        if ([value isKindOfClass:[NSDictionary class]]) {
            ...
            NSDictionary *dictionaryValue = [ALTUtil convertDictionaryValues:(NSDictionary *)value];
            [convertedDictionary setObject:dictionaryValue forKey:key];
        } else if ([value isKindOfClass:[NSDate class]]) {
            // Format date to our custom format
            NSString *dateStingValue = [ALTUtil formatDate:value];
...
+ (NSURL *)convertUniversalLink:(NSURL *)url scheme:(NSString *)scheme {
    ...
    if ([ALTUtil isNull:scheme] || [scheme length] == 0) {
        [logger warn:@"Non-empty scheme required, using the scheme \"AlltrackUniversalScheme\""];
        scheme = kDefaultScheme;
    }
    NSString *urlString = [url absoluteString];
    if ([ALTUtil isNull:urlString]) {
    ...
    NSString *finalTailSubString = [ALTUtil removeOptionalRedirect:tailSubString];
    ...
    if ([ALTUtil isNull:extractedUrl]) {
...
+ (NSString *)removeOptionalRedirect:(NSString *)tailSubString {
    ...
    if (optionalRedirectRegex == nil) {
        [ALTAlltrackFactory.logger error:@"Remove Optional Redirect regex not correctly configured"];
    ...
    if ([optionalRedirectmatches count] == 0) {
        [logger debug:@"Universal link does not contain option alltrack_redirect parameter"];
        return tailSubString;
    }
    if ([optionalRedirectmatches count] > 1) {
        [logger error:@"Universal link contains multiple option alltrack_redirect parameters"];
        return tailSubString;
    }
...
+ (NSTimeInterval)waitingTime:(NSInteger)retries
              backoffStrategy:(ALTBackoffStrategy *)backoffStrategy {
    ...
    double randomRange = [ALTUtil randomInRange:backoffStrategy.minRange maxRange:backoffStrategy.maxRange];
    // Apply jitter factor
...
+ (void)launchInMainThread:(NSObject *)receiver
    ...
    if (ALTAlltrackFactory.testing) {
        [ALTAlltrackFactory.logger debug:@"Launching in the background for testing"];
...
+ (void)launchInMainThreadWithInactive:(isInactiveInjected)isInactiveblock {
    dispatch_block_t block = ^void(void) {
        __block BOOL isInactive = [ALTUtil isInactive];
        isInactiveblock(isInactive);
    };
    if ([ALTUtil isMainThread]) {
        block();
        return;
    }
    if (ALTAlltrackFactory.testing) {
...
+ (BOOL)isValidParameter:(NSString *)attribute
    ...
    if ([ALTUtil isNull:attribute]) {
        [ALTAlltrackFactory.logger error:@"%@ parameter %@ is missing", parameterName, attributeType];
        return NO;
    }
    if ([attribute isEqualToString:@""]) {
        [ALTAlltrackFactory.logger error:@"%@ parameter %@ is empty", parameterName, attributeType];
        return NO;
    }
    return YES;
}
...
+ (NSDictionary *)mergeParameters:(NSDictionary *)target
    ...
    [source enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSString *obj, BOOL *stop) {
        ...
        if (oldValue != nil) {
            [ALTAlltrackFactory.logger warn:@"Key %@ with value %@ from %@ parameter was replaced by value %@",
...
+ (BOOL)deleteFileWithName:(NSString *)fileName {
    NSString *documentsFilePath = [ALTUtil getFilePathInDocumentsDir:fileName];
    NSString *appSupportFilePath = [ALTUtil getFilePathInAppSupportDir:fileName];
    BOOL deletedDocumentsFilePath = [ALTUtil deleteFileInPath:documentsFilePath];
    BOOL deletedAppSupportFilePath = [ALTUtil deleteFileInPath:appSupportFilePath];
...
#pragma clang diagnostic pop
    ...
    NSString *keyRm = [NSString stringWithFormat:@"%@%@",
                        [NSString altJoin:@"remove", @"item", @"at", @"path", @":", nil],
                        [NSString altJoin:@"error", @":", nil]];
    ...
    if (!deleted) {
        [[ALTAlltrackFactory logger] verbose:@"Unable to delete file at path %@", filePath];
    }
    if (error) {
        [[ALTAlltrackFactory logger] error:@"Error while deleting file at path %@", filePath];
    }
    return deleted;
}
...
#pragma clang diagnostic pop
    if ([sharedUIApplication respondsToSelector:openUrlSelector]) {
        /*
         ...
         if (!success) {
         [ALTAlltrackFactory.logger error:@"Unable to open deep link (%@)", deepLinkUrl];
         }
         }];
         */
...
#pragma clang diagnostic pop
        if (!success) {
            [ALTAlltrackFactory.logger error:@"Unable to open deep link without completionHandler (%@)", deepLinkUrl];
        }
    }
#endif
}
...
+ (BOOL)checkAttributionDetails:(NSDictionary *)attributionDetails {
    if ([ALTUtil isNull:attributionDetails]) {
        return NO;
    }

    NSDictionary *details = [attributionDetails objectForKey:@"Version3.1"];
    if ([ALTUtil isNull:details]) {
        return YES;
    }

    // Common fields for both iAd3 and Apple Search Ads
    if (![ALTUtil contains:details key:@"iad-org-name" value:@"OrgName"] ||
        ![ALTUtil contains:details key:@"iad-campaign-id" value:@"1234567890"] ||
        ![ALTUtil contains:details key:@"iad-campaign-name" value:@"CampaignName"] ||
        ![ALTUtil contains:details key:@"iad-lineitem-id" value:@"1234567890"] ||
        ![ALTUtil contains:details key:@"iad-lineitem-name" value:@"LineName"]) {
        [ALTAlltrackFactory.logger debug:@"iAd attribution details has dummy common fields for both iAd3 and Apple Search Ads"];
        return YES;
    }
    // Apple Search Ads fields
    if ([ALTUtil contains:details key:@"iad-adgroup-id" value:@"1234567890"] &&
        [ALTUtil contains:details key:@"iad-keyword" value:@"Keyword"] && (
            [ALTUtil contains:details key:@"iad-adgroup-name" value:@"AdgroupName"] ||
            [ALTUtil contains:details key:@"iad-adgroup-name" value:@"AdGroupName"]
        )) {
        [ALTAlltrackFactory.logger debug:@"iAd attribution details has dummy Apple Search Ads fields"];
        return NO;
    }
    // iAd3 fields
    if ([ALTUtil contains:details key:@"iad-adgroup-id" value:@"1234567890"] &&
        [ALTUtil contains:details key:@"iad-creative-name" value:@"CreativeName"]) {
        [ALTAlltrackFactory.logger debug:@"iAd attribution details has dummy iAd3 fields"];
        return NO;
    }
...
+ (BOOL)isDeeplinkValid:(NSURL *)url {
    ...
    if (excludedDeeplinkRegex == nil) {
        [ALTAlltrackFactory.logger error:@"Excluded deep link regex not correctly configured"];
        return NO;
    }
    ...
    if ([matches count] > 0) {
        [ALTAlltrackFactory.logger debug:@"Deep link (%@) processing skipped", urlString];
        return NO;
    }
...
+ (BOOL)trackingEnabled {
    ...
    Class adSupportClass = [ALTUtil adSupportManager];
...
+ (NSString *)idfv {
    Class class = NSClassFromString([NSString altJoin:@"U", @"I", @"device", nil]);
    if (class == nil) {
        return nil;
    }
    NSString *keyCd = [NSString altJoin:@"current", @"device", nil];
...
#pragma clang diagnostic pop
    NSString *keyIfv = [NSString altJoin:@"identifier", @"for", @"vendor", nil];
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    id dev = [class performSelector:selCd];
#pragma clang diagnostic pop
    NSString *keyM = [NSString altJoin:@"model", nil];
...
+ (NSUInteger)startedAt {
    ...
    NSString *m = [[NSString altJoin:@"s", @"ys", @"ct", @"l", nil] lowercaseString];
...
+ (void)checkForiAd:(ALTActivityHandler *)activityHandler queue:(dispatch_queue_t)queue {
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    ...
    BOOL iAdInformationAvailable = [ALTUtil setiAdWithDetails:activityHandler
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    id dev = [class performSelector:selCd];
#pragma clang diagnostic pop
    NSString *keySv = [NSString altJoin:@"system", @"version", nil];
...
#pragma clang diagnostic pop
                NSString *keyChk = [NSString stringWithFormat:@"%@%@",
                        [NSString altJoin:@"attributes", @"of", @"item", @"at", @"path", @":", nil],
                        [NSString altJoin:@"error", @":", nil]];
                ...
                if ([man respondsToSelector:selChk]) {
                    ...
                    CFStringRef *indexRef = dlsym(RTLD_SELF, [[NSString altJoin:@"N", @"S", @"file", @"creation", @"date", nil] UTF8String]);
    ...
    return [ALTUtil formatDate:installTime];
}
...
+ (NSString *)getPersistedRandomToken {
    ...
    keychainItem[(__bridge id)kSecAttrAccount] = @"alltrack_uuid";
...
+ (BOOL)setPersistedRandomToken:(NSString *)randomToken {
    ...
    if (status != noErr) {
        [[ALTAlltrackFactory logger] warn:@"Primary dedupe token unsuccessfully written"];
        return NO;
    } else {
        NSString *persistedRandomToken = [ALTUtil getPersistedRandomToken];
        if ([randomToken isEqualToString:persistedRandomToken]) {
            [[ALTAlltrackFactory logger] debug:@"Primary dedupe token successfully written"];
            return YES;
        } else {
            return NO;
        }
